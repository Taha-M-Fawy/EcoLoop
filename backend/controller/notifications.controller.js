...
const getNotificationById = (req, res) => {
    getNotificationByIdService(req.params.id, req.user.id)
      .then((notification) => {
        if (!notification) {
...
const updateNotification = (req, res) => {
    updateNotificationService(req.params.id, req.user.id, req.body)
    .then((notification) => {
        if (!notification) {
        return res.status(404).json({ message: "Notification not found" });
        }
...
const deleteNotification = (req, res) => {
    deleteNotificationService(req.params.id, req.user.id)
      .then((notification) => {
        if (!notification) {
          return res.status(404).json({ message: "Notification not found" });
        }
